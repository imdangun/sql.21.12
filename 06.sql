...
--  과제] 위 문장에서, and 를 쓰지 않고, 같은 데이터를 읽어라.
select e.employee_id, e.last_name, e.department_id,
    d.department_id, d.location_id
from employees e join departments d
on e.department_id = d.department_id
...
--1.
-- 주소를 출력한다.
-- locations 과 countries 테이블을 사용한다.
-- location ID, street address, city, state or province, country 를 출력한다.
...
--3.
-- Toronto에 위치한 부서에서 근무하는 사원들을 찾는다.
-- 이름, 직업, 부서번호, 부서명을 출력한다.
...
--4.
-- 사원들의 이름, 사원번호, 매니저의 이름, 사원번호를 출력한다.
-- 칼럼명은 Employee, Emp#, Manager, Mgr# 로 한다.
...
-- 5.
-- 위 문제에서, 매니저가 없는 King 사원도 출력되도록 한다.
-- 사원번호 순으로 정렬한다.
select w.last_name "Employee", w.employee_id "Emp#",
    m.last_name "Manager", m.employee_id "Mgr#"
...
-- 6.
-- 같은 부서에서 일하는 사원들의 이름, 부서번호, 동료의 이름을 출력한다.
...
-- 8.
-- 매니저보다 먼저 입사한 사원들을 찾는다.
-- 사원의 이름, 사원 입사일, 매니저의 이름, 매니저 입사일을 출력한다.
